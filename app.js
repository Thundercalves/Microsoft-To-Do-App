...
function addTask(taskText, isImportant = false, isDone = false) {
  ...
  // Retrieve the list elements for tasks and completed tasks from local storage
  ...
  // To enable task editing on double click and to stop editing when focus is lost
  ...
  // Apply styles based on whether the task is marked as important or not 
  ...
  // Append the checkbox, task wrapper (text + label), and the star icon to the list item --> basically to show the task
  li.appendChild(checkbox);
  li.appendChild(taskWrapper);
  li.appendChild(star);

  // Apply styles based on the task's importance 
  if (isImportant) {
    li.classList.add('important');
  }

  // Apply styles based on whether task is done 
  if (isDone) {
    li.classList.add('done');
  }
  // If a task is done, it's either added to the beginning of the completed list or at the end if the list is empty.
  //If a task is not done, it's either added to the beginning of the task list or at the end if the list is empty.
